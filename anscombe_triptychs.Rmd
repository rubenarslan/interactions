...
model <- list()
model$assumed <- brm(Y ~ X*M, data = cases$assumed, cores = 4, backend = "cmdstanr")
model$floor <- brm(Y ~ X*M, data = cases$floor, cores = 4, backend = "cmdstanr")
library(cowplot)
plot_grid(pp_check(model$assumed) + guides(color = F), pp_check(model$floor) + theme(legend.position = c(1,0.9), legend.justification = "right"), pp_check(model$residual) + guides(color = F), nrow = 1, labels = "AUTO", label_size = 6)
ggsave("three_dgms_ppc.pdf", width = 5, height = 2)
ggsave("three_dgms_ppc.png", width = 5, height = 2)
model$residual <- brm(Y ~ X*M_f, data = cases$residual, cores = 4, backend = "cmdstanr")
...
ggplot(three_cases, 
       aes(X, Y)) +
  geom_point(aes(color = M), alpha = 0.1) +
  geom_line(aes(X, estimate__), color = "black") +
